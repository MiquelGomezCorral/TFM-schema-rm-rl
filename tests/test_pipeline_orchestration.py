...
class Engine:
    def __init__(self): self.environment=ENV; self.tasks=[]; self.rms=[]
...
class OrchestrationTests(unittest.TestCase):
    ...
    def execute(self, tasks=("finish",), **flags):
        ...
        with patch.object(g,"_setup_environment_and_engine",return_value=(ENV,self.engine)), patch.object(g,"_derive_output_paths",return_value=tuple(Path(f"out-{i}.rm") for i in range(len(tasks)))), patch.object(g,"_save_results",self.saved), patch.object(g,"propose_task",side_effect=lambda e,t,engine,history="": prop(t)), patch.object(g,"compile_proposal",return_value=result()), patch.object(g,"print_separator"), patch.object(g,"print_color"): return g.generate_rm(config)
    ...
    def test_task_rejection_skips_compile_and_history(self):
        self.engine.task_results=[CriticResult(False,"fix task"),CriticResult(True,"ok")]; self.engine.rm_results=[CriticResult(True,"ok")]; histories=[]
        with patch.object(g,"_setup_environment_and_engine",return_value=(ENV,self.engine)), patch.object(g,"_derive_output_paths",return_value=(Path("out.rm"),)), patch.object(g,"_save_results",self.saved), patch.object(g,"propose_task",side_effect=lambda e,t,engine,history="": histories.append(history) or prop(t)), patch.object(g,"compile_proposal",return_value=result()) as compile_mock, patch.object(g,"print_separator"), patch.object(g,"print_color"): self.assertEqual(g.generate_rm(Configuration(environment="demo.md",tasks=["finish"],output="out.rm")),0)
        self.assertEqual(compile_mock.call_count,1); self.assertIn("fix task",histories[1]); self.assertIn("task critic",histories[1]); self.assertIn('"source": "task critic"',histories[1])
    def test_rm_critic_failure_keeps_compiled_rm_in_history(self):
        histories=[]; self.engine.task_results=[CriticResult(True,"ok")]*2; self.engine.rm_results=[RetryableEngineError("critic unavailable"),CriticResult(True,"ok")]
        compiled=[SimpleNamespace(text="rm-before-retry",proposal=prop()),result()]
        with patch.object(g,"_setup_environment_and_engine",return_value=(ENV,self.engine)), patch.object(g,"_derive_output_paths",return_value=(Path("out.rm"),)), patch.object(g,"_save_results",self.saved), patch.object(g,"propose_task",side_effect=lambda e,t,engine,history="": histories.append(history) or prop(t)), patch.object(g,"compile_proposal",side_effect=compiled), patch.object(g,"print_separator"), patch.object(g,"print_color"):
            self.assertEqual(g.generate_rm(Configuration(environment="demo.md",tasks=["finish"],output="out.rm")),0)
        self.assertIn("rm-before-retry",histories[1]); self.assertIn("critic unavailable",histories[1]); self.assertIn('"source": "RM critic"',histories[1])
    def test_rm_rejection_recompiles_and_history(self):
        self.engine.task_results=[CriticResult(True,"ok")]*2; self.engine.rm_results=[CriticResult(False,"fix rm"),CriticResult(True,"ok")]; histories=[]; compiled=[SimpleNamespace(text="rm-1",proposal=prop()),result()]
        with patch.object(g,"_setup_environment_and_engine",return_value=(ENV,self.engine)), patch.object(g,"_derive_output_paths",return_value=(Path("out.rm"),)), patch.object(g,"_save_results",self.saved), patch.object(g,"propose_task",side_effect=lambda e,t,engine,history="": histories.append(history) or prop(t)), patch.object(g,"compile_proposal",side_effect=compiled), patch.object(g,"print_separator"), patch.object(g,"print_color"): self.assertEqual(g.generate_rm(Configuration(environment="demo.md",tasks=["finish"],output="out.rm")),0)
        self.assertIn("rm-1",histories[1]); self.assertIn("fix rm",histories[1]); self.assertIn('"source": "RM critic"',histories[1])
    ...
    def test_multiple_task_output_names_are_numbered(self):
        ...
        self.assertEqual([path.name for path in g._derive_output_paths(config)],["batch-1.rm","batch-2.rm"])
    def test_blank_task_fails_before_setup_or_generation(self):
        config=Configuration(environment="demo.md",tasks=["  "],output="out.rm")
        with patch.object(g,"_setup_environment_and_engine") as setup, patch.object(g,"propose_task") as propose, patch.object(g,"print_separator"):
    ...
    def test_retryable_immediate_and_mona_failures(self):
        ...
        with patch.object(g,"_setup_environment_and_engine",return_value=(ENV,self.engine)), patch.object(g,"_derive_output_paths",return_value=(Path("out.rm"),)), patch.object(g,"_save_results",self.saved), patch.object(g,"propose_task",side_effect=[RetryableEngineError("rate"),prop()]), patch.object(g,"compile_proposal",return_value=result()), patch.object(g,"print_separator"), patch.object(g,"print_color"): self.assertEqual(g.generate_rm(Configuration(environment="demo.md",tasks=["finish"],output="out.rm",task_critic=False)),0)
        with patch.object(g,"_setup_environment_and_engine",side_effect=ImmediateEngineError("denied")), patch.object(g,"print_separator"): self.assertEqual(g.generate_rm(Configuration(environment="demo.md",tasks=["finish"],output="out.rm")),1)
        self.engine.task_results=[CriticResult(True,"ok")]
        with patch.object(g,"_setup_environment_and_engine",return_value=(ENV,self.engine)), patch.object(g,"_derive_output_paths",return_value=(Path("out.rm"),)), patch.object(g,"propose_task",return_value=prop()), patch.object(g,"compile_proposal",side_effect=RuntimeError("MONA failed")), patch.object(g,"print_separator"), patch.object(g,"print_color"): self.assertEqual(g.generate_rm(Configuration(environment="demo.md",tasks=["finish"],output="out.rm")),1)
    def test_compiler_failure_is_recorded_as_compiler_source(self):
        histories=[]; self.engine.rm_results=[CriticResult(True,"ok")]
        with patch.object(g,"_setup_environment_and_engine",return_value=(ENV,self.engine)), patch.object(g,"_derive_output_paths",return_value=(Path("out.rm"),)), patch.object(g,"_save_results",self.saved), patch.object(g,"propose_task",side_effect=lambda e,t,engine,history="": histories.append(history) or prop(t)), patch.object(g,"compile_proposal",side_effect=[ValueError("bad candidate"),result()]), patch.object(g,"print_separator"), patch.object(g,"print_color"):
            self.assertEqual(g.generate_rm(Configuration(environment="demo.md",tasks=["finish"],output="out.rm",task_critic=False)),0)
        self.assertIn('"source": "compiler"',histories[1])
    def test_malformed_proposal_is_recorded_as_generator_source(self):
        histories=[]; self.engine.rm_results=[CriticResult(True,"ok")]
        def malformed_then_valid(*args, history=""):
            histories.append(history)
            if len(histories) == 1: raise ProposalValidationError("malformed")
            return prop()
        with patch.object(g,"_setup_environment_and_engine",return_value=(ENV,self.engine)), patch.object(g,"_derive_output_paths",return_value=(Path("out.rm"),)), patch.object(g,"_save_results",self.saved), patch.object(g,"propose_task",side_effect=malformed_then_valid), patch.object(g,"compile_proposal",return_value=result()), patch.object(g,"print_separator"), patch.object(g,"print_color"):
            self.assertEqual(g.generate_rm(Configuration(environment="demo.md",tasks=["finish"],output="out.rm",task_critic=False)),0)
        self.assertIn('"source": "generator"',histories[1])
    ...
    def test_structured_pipeline_critic_precedes_ltlf_materialization(self):
        ...
        with patch.object(g, "_setup_environment_and_engine", return_value=(ENV, engine)), \
            patch.object(g, "_derive_output_paths", return_value=(Path("out.rm"),)), \
            patch.object(g, "_save_results"), \
